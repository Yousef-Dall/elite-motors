      ...
      <div className="mx-auto max-w-7xl px-4 py-20 md:py-28 grid md:grid-cols-2 items-center gap-10">
        <motion.div variants={fade} initial="hidden" animate="show" className="space-y-6 text-start">
          ...
          <div className="flex flex-wrap items-center gap-3">
            <a href="#contact" className="group inline-flex items-center gap-2 px-5 py-3 rounded-2xl text-white font-semibold bg-gradient-to-r from-cyan-500 via-indigo-500 to-fuchsia-500 shadow-lg shadow-cyan-500/20 hover:shadow-cyan-500/30 transition">
              {t("hero.cta1")}
              <ArrowRight className={`h-4 w-4 transition ${isAr ? "-scale-x-100" : ""} group-hover:translate-x-0.5`} />
            </a>
            <a href="#services" className="inline-flex items-center gap-2 px-5 py-3 rounded-2xl border transition bg-white/70 border-black/10 hover:bg-white/90 dark:bg-white/5 dark:border-white/10 dark:hover:bg-white/10">
              {t("hero.cta2")}
            </a>
          </div>
          <div className="flex flex-wrap items-center gap-6 pt-2 text-neutral-600 dark:text-white/70">
        ...
        <motion.div initial={{ opacity: 0, scale: 0.98 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.8, ease: "easeOut" }} className="relative">
          <motion.div whileHover={{ y: -6 }} transition={{ type: "spring", stiffness: 240, damping: 18 }} className="relative aspect-[5/3] w-full overflow-visible rounded-3xl">
            <div className="relative h-full overflow-hidden rounded-3xl border shadow-2xl border-black/10 dark:border-white/10">
              <img src="/images/hero-lambo.jpg" alt="Elite Motors workshop build" width="1600" height="960" decoding="async" fetchPriority="high" className="absolute inset-0 z-10 h-full w-full object-cover" />
            </div>
            <div className="absolute -bottom-24 left-0 right-0 h-24 overflow-hidden rounded-t-3xl pointer-events-none">
              <img src="/images/hero-lambo.jpg" alt="" aria-hidden="true" width="1600" height="960" decoding="async"
                   className="w-full h-[180%] object-cover blur-[1px] opacity-35 saturate-75"
                   style={{ transform: "scaleY(-1)", WebkitMaskImage: "linear-gradient(to top, rgba(0,0,0,0.45), rgba(0,0,0,0))", maskImage: "linear-gradient(to top, rgba(0,0,0,0.45), rgba(0,0,0,0))" }} />
            </div>
