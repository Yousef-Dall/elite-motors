...
export default function Services() {
  ...
  // if translation key doesn't exist, fall back to english string in SERVICES
  const pick = (key, fallback) => (t(key) === key ? fallback : t(key));
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 text-start">
      ...
      <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
        {SERVICES.map((svc, i) => {
          const Icon = svc.icon;
          const title = pick(`services.items.${i}.title`, svc.title);
          const desc = pick(`services.items.${i}.desc`, svc.desc);
          const tag = pick(`services.items.${i}.tag`, svc.tag);
