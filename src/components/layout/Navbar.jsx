...
export default function Navbar() {
  ...
  const nav = useMemo(
    () => [
      { label: t("navbar.home"), href: "/", id: "home-link", route: true },
      { label: t("navbar.about"), href: "/#about", id: "about" },
      { label: t("navbar.services"), href: "/#services", id: "services" },
      { label: "Pricing", href: "/pricing", id: "pricing", route: true },
      { label: "Booking", href: "/booking", id: "booking", route: true },
      { label: "Blog", href: "/blog", id: "blog", route: true },
  ...
  const activeId = useScrollSpyAuto(nav.map((n) => n.id), {
    headerRef,
    lineRatio: 0.6,       // reading line lower => later switch
    switchOffsetPx: 220,   // require px inside next section
    switchFraction: 0.28,  // or ~28% of section height
    hysteresisPx: 120,     // keeps current section a bit longer
  });

  // --- Sync CSS variable for header height ---
  useEffect(() => {
  ...
  }, [lang]); // re-measure if language changes text width

  // --- Smooth scroll accounting for header height ---
  const handleNavClick = (e, href) => {
    const id = href?.startsWith("#") ? href.slice(1) : null;
    if (!id) return; // normal route link
    ...
    // Use measured header height with breathing space
    const headerH =
      parseFloat(
