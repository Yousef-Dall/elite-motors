...
export default function Gallery() {
  ...
  // localize captions if provided in dictionary, else null
  const slides = useMemo(
    () =>
      IMAGES.map((g, i) => ({
        ...g,
        title: t(`gallery.items.${i}.title`) || null,
        cap: t(`gallery.items.${i}.cap`) || null,
      })),
    [t]
  );

  // watch which slide is most in view to update dot indicator
  useEffect(() => {
    ...
    const io = new IntersectionObserver(
      (entries) => {
        ...
        if (vis) {
          const n = Number(vis.target.getAttribute("data-slide"));
  ...
  // helper to smoothly scroll to a given slide index
  const scrollTo = (i) => {
    const root = scrollRef.current;
    if (!root) return;
    const clamped = Math.max(0, Math.min(slides.length - 1, i));
    const el = root.querySelector(`[data-slide="${clamped}"]`);
    if (!el) return;

    const targetLeft =
      el.offsetLeft - (root.clientWidth - el.clientWidth) / 2;
  ...
  // drag / swipe handlers (pointer works for mouse & touch)
  ...
  const onPointerMove = (e) => {
    const root = scrollRef.current;
    if (!root || !isDownRef.current) return;

    const clientX = e.clientX ?? e.touches?.[0]?.clientX ?? 0;
    root.scrollLeft = scrollStartRef.current - (clientX - startXRef.current);
  };
  ...
  // manual pause booster after interaction
  function bumpPause() {
  ...
  useEffect(() => {
    const vis = () => {
      setPaused(
        document.hidden || Date.now() < pauseUntilRef.current
      );
    };
    ...
    <div className="mx-auto max-w-7xl px-4">
      ...
      <div className="mb-6 text-start">
        <div className="text-sm uppercase tracking-[0.25em] text-neutral-500 dark:text-white/50">
          {t("gallery.tag") || "Inside the Workshop"}
        </div>
      ...
      <div className="relative">
        ...
        <button
          ...
          className="absolute left-2 top-1/2 -translate-y-1/2 z-10 grid h-10 w-10 place-items-center
                     rounded-full bg-white/80 border border-black/10 shadow hover:bg-white
        ...
        <button
          ...
          className="absolute right-2 top-1/2 -translate-y-1/2 z-10 grid h-10 w-10 place-items-center
                     rounded-full bg-white/80 border border-black/10 shadow hover:bg-white
                     dark:bg-white/10 dark:border-white/10"
        >
        ...
        <div
          ...
          aria-label={t("gallery.title") || "Gallery"}
          ...
          className="scrollbar-hide relative flex snap-x snap-mandatory gap-6 overflow-x-auto
                     rounded-3xl border border-black/10 dark:border-white/10 p-4
          ...
          {slides.map((s, i) => (
            <figure
              ...
              className="relative snap-center shrink-0
                         w-[82%] sm:w-[70%] md:w-[55%] lg:w-[45%] xl:w-[38%]"
            >
          ...
          {slides.map((_, i) => {
            ...
            return (
              <button
                ...
                aria-label={`${t("gallery.title") || "Gallery"} – slide ${
                  i + 1
                }`}
                className={[
                  "h-2.5 rounded-full transition-all",
                  active
                    ? "w-8 bg-gradient-to-r from-cyan-500 via-indigo-500 to-fuchsia-500"
                    : "w-2.5 bg-white/30 dark:bg-white/20",
                ].join(" ")}
              />
            );
          })}
      ...
      {/* tiny keyframe helper for fade, kept from your version */}
      <style>{`
        @keyframes emfade {
